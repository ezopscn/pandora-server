...
func (r *InterfaceResult) Unwrap() interface{} {
	if r.Error != nil {
		global.SystemLog.Debug("Redis 缓存查询失败：", r.Error.Error())
	}
	return r.Result
}
...
func (r *InterfaceResult) UnwrapWithDefaultValue(v interface{}) interface{} {
	if r.Error != nil {
		global.SystemLog.Debug("Redis 缓存查询失败，返回默认值：", r.Error.Error())
		return v
	}
	return r.Result
}
...
func (r *InterfaceResult) UnwrapWithFunc(f func() interface{}) interface{} {
	if r.Error != nil {
		global.SystemLog.Debug("Redis 缓存查询失败，返回执行函数：", r.Error.Error())
		return f()
	}
	return r.Result
}
